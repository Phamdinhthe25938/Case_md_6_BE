...
<<<<<<< HEAD
public interface IPostEnterpriseRepo extends JpaRepository<PostEnterprise, Integer> {
...
<<<<<<< HEAD
    @Query(nativeQuery = true, value = "select * from case_module_6.post_enterprise where enterprise_id_enterprise =:idEnterprise")
    Page<PostEnterprise> findAllByEnterprise(@Param("idEnterprise") int idEnterprise);
...
//    lấy tất cả bài đăng của ngày hiện tại
    ...
    int sumAllPostEnterprise(@Param("id") int id);

    @Query(nativeQuery = true, value = "select count(id_post_enterprise)  from case_module_6.post_enterprise where enterprise_id_enterprise=:id and regime_id_regime = 1 ")
    int sumPostEnterPriseVip(@Param("id") int id);
